...
const buildRun = async (vNode, attributes, docxDocumentInstance) => {
  ...
  if (isVText(vNode)) {
    ...
    if (response) {
      ...
      console.log('test', vNode.properties.style);
      attributes.inlineOrAnchored = true;

      if(vNode.properties.style.float || vNode.properties.align){
        attributes.inlineOrAnchored = false;
        attributes.anchoredType = vNode.properties.style.float || vNode.properties.align
      }
